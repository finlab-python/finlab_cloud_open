...
def create_features(financial_statement):
    ...
    繼續營業部門稅前純益 = data.get('繼續營業單位稅前淨利_淨損')
    ...
    稅前息前折舊前淨利 = 繼續營業部門稅前純益.fillna(0) + 財物成本.fillna(0) + 折舊.fillna(0) + 攤提.fillna(0)
    稅前息前折舊前淨利[稅前息前折舊前淨利 == 0] = np.nan

    稅前息前淨利 = 繼續營業部門稅前純益.fillna(0) + 財物成本.fillna(0)
    ...
    研究發展費用率 = 研究發展費 / 營業費用 * 100
    現金流量比率 = 營運現金流 / 流動負債 * 100
    稅率 = (所得稅費用 / 繼續營業部門稅前純益 * 100)
    稅率[(繼續營業部門稅前純益 <= 0) | (所得稅費用 <= 0)] = 0

    # 償債能力指標
    每股營業額 = 營業收入淨額 / 股本 * 10
    每股營業利益 = 營業利益 / 股本 * 10
    每股現金流量 = 營運現金流 / 股本 * 10
    每股稅前淨利 = 繼續營業部門稅前純益 / 股本 * 10
